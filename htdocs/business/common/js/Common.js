...
Common = (function() {
  function Common(opt) {
    ...
    this.windowScrollHandler = __bind(this.windowScrollHandler, this);
    this.initContentNav = __bind(this.initContentNav, this);
  ...
  Common.prototype.initSmoothScroll = function() {
    return $('a[href^=#]').not("").click((function(_this) {
      return function(_t) {
        ...
        _this.smoothScrollTo(target, speed, -20);
        return false;
      };
    })(this));
  };
  ...
  Common.prototype.initSiteCommon = function() {
    ...
    if (this.isPC) {
      ...
      this.fontResize(txtCookie);
      return this.initContentNav();
    }
  };
  ...
  Common.prototype.initContentNav = function() {
    this.cnav = $("#content-nav");
    if (this.cnav.position()) {
      this.navMarginTop = 20;
      this.navDefTop = this.cnav.position().top;
      this.navFixPos = this.navDefTop - this.navMarginTop;
      this.navLimitTop = $("html").height();
      return $(window).bind("scroll", this.windowScrollHandler);
    }
  };

  Common.prototype.windowScrollHandler = function() {
    var st;
    st = $(window).scrollTop();
    if (st > this.navFixPos) {
      if (st > this.navLimitTop) {
        return this.cnav.css({
          "position": "absolute",
          "top": this.navLimitTop
        });
      } else {
        return this.cnav.css({
          "position": "fixed",
          "top": this.navMarginTop
        });
      }
    } else if (st <= this.navFixPos) {
      return this.cnav.css({
        "position": "absolute",
        "top": this.navDefTop
      });
